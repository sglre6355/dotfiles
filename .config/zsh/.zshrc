...
_arduino-cli()
{
    ...
    while IFS='\n' read -r comp; do
        ...
        if [ "${comp[1,$endIndex]}" = "$activeHelpMarker" ];then
            ...
            if [ -n "$comp" ]; then
                compadd -x "${comp}"
                __arduino-cli_debug "ActiveHelp will need delimiter"
                hasActiveHelp=1
            fi
    ...
    if [ $hasActiveHelp -eq 1 ]; then
        if [ ${#completions} -ne 0 ] || [ $((directive & shellCompDirectiveNoFileComp)) -eq 0 ]; then
            __arduino-cli_debug "Adding activeHelp delimiter"
            compadd -x "--"
            hasActiveHelp=0
        fi
    fi
